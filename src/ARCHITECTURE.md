...
- Per-line row warnings: `SyncedLyricsEditor.vue`'s `rowClass(index)` washes each line row with a full-row warning for two independent per-line conditions (only when the row isn't selected/hovered/editing — those states still win). **Overlap** (line is part of an overlapping pair — `overlappingLineIndexes`, a single-pass sweep that relies on the sorted-by-`start_ms` invariant) → solid rose (`bg-rose-100 dark:bg-rose-950/40`). **Desync** (an active transliteration system's line-level reading disagrees with its cue readings — `isLineOutOfSync`, gated on `selectedTransliterationSystem`) → solid amber (`bg-amber-100 dark:bg-amber-900/40`). **Both at once** → 45° equal-width amber/rose diagonal stripes via the scoped `.lyric-row-warn-both` class in `SyncedLyricsLineRow.vue` (a `repeating-linear-gradient` no Tailwind utility can express, with a `.dark` variant using the same colors)
...
- Transliteration systems: the word-timing lane manages the `metadata.transliterations` declaration (an array of `{ id, system }` where `id` is an internal lyricsfile key never transported and `system` is a BCP-47 locale). Named presets (`TRANSLITERATION_PRESETS` in `useEditLyricsV2Document.js`) map friendly labels to BCP-47 tags — Furigana→`ja-Hrkt`, Romaji→`ja-Latn`, Pinyin→`zh-Latn-pinyin`, Zhuyin→`zh-Bopo`, Romaja→`ko-Latn` — plus a Custom… option that takes a raw BCP-47 tag. `deriveTransliterationId` derives the id from the preset name (Furigana→`hira`, Romaji→`romaji`, …) or a slug of a custom name, suffixing collisions numerically (`base`, `base2`, …). A transliteration controls bar sits above the spectrograph/timeline and is always rendered (no longer gated on system count): with zero systems it shows a "No transliteration" placeholder plus an add button; with ≥1 system it shows a `<select>` for the active system followed by pencil (inline re-tag), trash (remove), and add buttons. The add button opens the "Add reading system" popover whose preset list is ordered so presets matching the document's `metadata.language` primary subtag float first (ja→Furigana/Romaji, zh→Pinyin/Zhuyin, ko→Romaja). Inline re-tag replaces the `<select>` itself with a text `<input>` (confirm/cancel) rather than opening a separate field; the trash button removes the active system. Editing re-keys every existing line/word reading from the old id to the newly-derived id; removing deletes all readings stored under that id (both behind a `window.confirm`). Empty reading maps collapse to `undefined` so `{}` is never retained. State (`selectedTransliterationSystem`, `declaredTransliterations`) and mutations (`addTransliterationSystem`, `editTransliterationSystem`, `removeTransliterationSystem`) live in `useEditLyricsV2Document.js` and are threaded down through `EditLyricsV2.vue`→`SyncedLyricsEditor.vue`→`SyncedWordTimingLane.vue` via props/events
- Per-word reading editor (transliteration track + inline input): with a reading system active, the lane renders a dedicated **transliteration track** row directly above the main timing track (same horizontal scale), growing the lane height by ~1.75rem to fit it; each word's stored reading is shown as a label in its slot there, and the word-boundary lines continue upward into the track as faint (desaturated) 1px lines so the track and timeline read as one connected surface. In the segment (`SyncedWordTimingSegment.vue`), a kanji/hanzi word missing a reading (`needsTransliteration` in `word-tokenizer.js`) gets only a subtle amber "add here" underline — no box; kana/ASCII words show nothing. Exactly one word per line is editable at a time: clicking a highlight (or `Tab`/`Enter`-traversing into it) opens a single inline `<input>` that floats up into the transliteration track (`top: -1.75rem`) to occupy that word's base. Typing persists to `word.transliteration[systemId]` through the existing `update:words`→`updateLineWords` path — a blank field deletes the key and an emptied map collapses so `{}` is never stored; timing is never touched. `Tab`/`Enter` advance to the next word still needing a reading (wrapping once, then closing). The active word is tracked lane-local as `activeWordIndex` in `SyncedWordTimingLane.vue` (one line is shown at a time) and reset on line switch, active-system change, or when the visible word set shrinks; the outgoing field's async blur only clears state when it is still the active word, so a `Tab`-jump is never clobbered
- Line-level readings & cue↔line sync: `line.transliteration[id]` holds the whole-line reading — the source of truth for the inter-word spacing that romanization systems (romaji/pinyin/romaja) carry but cue readings omit. `src/utils/transliteration-sync.js` owns the sync logic: `propagateCueEditToLine` surgically patches only the edited cue's span inside the line-level string via non-whitespace positional anchoring (inserting the cue's `effectiveReading`, i.e. the ruby with okurigana folded in by `composeReading`, which ignores whitespace in the cue text so a tokenizer-kept trailing space like `日 ` never leaks into the line), and **only when the line already agrees with the cues** (its precondition is exactly the negation of `isLineOutOfSync`), so it never collapses spacing (`"otona ni naru"` never becomes `"otonaninaru"`) nor clobbers a manual edit; when it can't align cleanly it declines and leaves the warning to surface the divergence. Structural edits (split/merge/reset — any word-count change) skip propagation entirely. It's centralized in `useEditLyricsV2Document.updateLineWords`, which diffs old-vs-new words to find the single changed `(system, index)`. Direct editing goes through `updateLineTransliteration` (verbatim; an all-whitespace value clears the key), and `generateLineTransliterationFromCues` is the explicit, opt-in rebuild (space-joined for romanization via `isRomanizationSystem`, no separator for syllabaries). UI: `SyncedWordTimingLane.vue` renders an editable line-level lane directly below the word-timing timeline, with an amber out-of-sync warning (romanization only, via `isLineOutOfSync`) and a Generate-from-cues button; `SyncedLyricsLineRow.vue` shows the line-level reading when a system is active, or faded main text when the line has none yet (so lines still needing a reading are visible at a glance)
...
Utils: `src/utils/` (parsing, linting, transliteration sync), Composables: `composables/edit-lyrics-v2/`, `composables/export.js`. Default word timing tokenization uses backend `segment_words` (Charabia), with frontend tokenizer fallback.
