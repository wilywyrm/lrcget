...
- Transliteration systems: the word-timing lane manages the `metadata.transliterations` declaration (an array of `{ id, system }` where `id` is an internal lyricsfile key never transported and `system` is a BCP-47 locale). Named presets (`TRANSLITERATION_PRESETS` in `useEditLyricsV2Document.js`) map friendly labels to BCP-47 tags — Furigana→`ja-Hrkt`, Romaji→`ja-Latn`, Pinyin→`zh-Latn-pinyin`, Zhuyin→`zh-Bopo`, Romaja→`ko-Latn` — plus a Custom… option that takes a raw BCP-47 tag. `deriveTransliterationId` derives the id from the preset name (Furigana→`hira`, Romaji→`romaji`, …) or a slug of a custom name, suffixing collisions numerically (`base`, `base2`, …). An "Add reading system" popover is always available in the lane header; the preset list is ordered so presets matching the document's `metadata.language` primary subtag float first (ja→Furigana/Romaji, zh→Pinyin/Zhuyin, ko→Romaja). A `<select>` for choosing the active system appears left of the spectrograph only when ≥2 systems are declared, with inline edit (re-tag) and remove affordances. Editing re-keys every existing line/word reading from the old id to the newly-derived id; removing deletes all readings stored under that id (both behind a `window.confirm`). Empty reading maps collapse to `undefined` so `{}` is never retained. State (`selectedTransliterationSystem`, `declaredTransliterations`) and mutations (`addTransliterationSystem`, `editTransliterationSystem`, `removeTransliterationSystem`) live in `useEditLyricsV2Document.js` and are threaded down through `EditLyricsV2.vue`→`SyncedLyricsEditor.vue`→`SyncedWordTimingLane.vue` via props/events
- Per-word reading editor (quiet ruby stack): with a reading system active, each word segment (`SyncedWordTimingSegment.vue`) shows one of four low-noise states. A word that already has a reading under the active system floats a muted **static ruby** above its base (click to edit); a kanji/hanzi word missing a reading (`needsTransliteration` in `word-tokenizer.js`) gets only a subtle amber "add here" underline — no box; kana/ASCII words show nothing. Exactly one word per line is editable at a time: clicking a ruby/highlight (or `Tab`/`Enter`-traversing into it) opens a single inline `<input>` floating just above that word's base. Typing persists to `word.transliteration[systemId]` through the existing `update:words`→`updateLineWords` path — a blank field deletes the key and an emptied map collapses so `{}` is never stored; timing is never touched. `Tab`/`Enter` advance to the next word still needing a reading (wrapping once, then closing). The active word is tracked lane-local as `activeWordIndex` in `SyncedWordTimingLane.vue` (one line is shown at a time) and reset on line switch, active-system change, or when the visible word set shrinks; the outgoing field's async blur only clears state when it is still the active word, so a `Tab`-jump is never clobbered
